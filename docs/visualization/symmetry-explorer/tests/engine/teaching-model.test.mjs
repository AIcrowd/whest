...
} from '../../src/engine/teachingModel.js';
...
test('PSEUDOCODE_LINES defines a stable teaching scaffold', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      '# evaluation_cost counts one product evaluation per G-orbit representative.',
      'evaluation_cost = 0',
      '# reduction_cost counts one update per distinct projected output bin.',
      'reduction_cost = 0',
      '',
...
test('getFocusedLines maps each walkthrough step to the relevant pseudocode lines', () => {
  ...
  assert.deepEqual(getFocusedLines('burnside'), [1, 2, 3, 4, 8, 9, 10, 11, 12, 13]);
  assert.deepEqual(getFocusedLines('cost'), [5, 6, 15, 16, 17, 18, 19, 20]);
  assert.deepEqual(
...
test('tokenizePseudocodeLine marks keywords, state variables, functions, and numbers', () => {
  ...
  assert.deepEqual(tokenizePseudocodeLine('evaluation_cost = 0'), [
    { text: 'evaluation_cost', kind: 'state' },
    { text: ' ', kind: 'plain' },
    { text: '=', kind: 'plain' },
    { text: ' ', kind: 'plain' },
    { text: '0', kind: 'number' },
