...
> **whest is NOT NumPy.** A global default budget auto-activates on import, but explicit `BudgetContext` is recommended for control.
> Some operations cost FLOPs, some are free, and 35 are blocked entirely.

## Key Constraints

- A global default budget auto-activates when whest is imported — no explicit `BudgetContext` required for quick scripts
- For precise budget control and namespacing, use an explicit `BudgetContext`
...
| Category | Count | Cost Formula | Typical Weight |
|----------|-------|-------------|----------------|
| Free | 80 | $0$ | — |
| Unary | 71 | $\text{numel}(\text{output})$ | 16.0 |
| Binary | 47 | $\text{numel}(\text{output})$ | 1.0 |
| Reduction | 37 | $\text{numel}(\text{input})$ | 1.0 |
| Custom | 238 | varies | varies |
| Blocked | 35 | N/A | — |

Effective FLOPs charged = Analytical Cost × Weight. Weights are loaded from `src/whest/data/weights.csv` (column "Active Weight") and can be customized via `WHEST_WEIGHTS_FILE` or `we.load_weights()`.
...
| `linalg.cross` | delegates to `cross` | Delegates to `we.cross` which charges `numel(output)` FLOPs. |
...
| `linalg.matmul` | delegates to `matmul` | Delegates to `we.matmul` which charges `m*k*n` FLOPs (FMA=1). |
...
| `linalg.outer` | delegates to `outer` | Delegates to `we.outer` which charges `m*n` FLOPs. |
...
| `linalg.tensordot` | delegates to `tensordot` | Delegates to `we.tensordot` which charges FLOPs based on contraction. |
...
| `linalg.vecdot` | delegates to `vecdot` | Delegates to `we.vecdot` which charges `2*n` FLOPs. |
...
| `roots` | $n^3$ | Return roots of polynomial with given coefficients. Cost: $n^3$ (companion matrix eig, n=degree). Weight: 16.0. |
