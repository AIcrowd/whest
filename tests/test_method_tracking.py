...
def test_reduction_method_matches_function(name, op):
    """a.foo() must produce the same FLOP count as flops.foo(a)."""
...
def test_searchsorted_method_tracks():
    ...
    are the only thing in ``bc.flops_used``. Calling ``flops.sort`` /
    ``flops.random.randn`` outside a budget would itself raise.
    """
    sorted_arr = np.sort(np.random.randn(8)).view(flops.ndarray)
    with flops.BudgetContext(flop_budget=int(1e9)) as bc:
...
def test_method_positional_dtype_tracks():
    ...
    a = flops.random.randn(4, 5).astype(np.float32)
    with flops.BudgetContext(flop_budget=int(1e9)) as bc:
...
def test_method_positional_out_argmax_tracks_and_returns_out():
    ...
    a = flops.random.randn(4, 5)
    out = flops.zeros(5, dtype=np.intp)
    with flops.BudgetContext(flop_budget=int(1e9)) as bc:
...
def test_no_symmetry_downgrades_to_whest_array_via_diagonal():
    """Issue #62: ``flops.diagonal`` of a 2-axis-symmetric matrix has no
...
def test_symmetric_getitem_full_reduction_via_indexing_returns_whest_array():
    ...
    assert isinstance(row, flops.ndarray)
    assert row.shape == (4,)
