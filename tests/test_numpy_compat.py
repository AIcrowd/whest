"""Verify that flopscope produces identical results to NumPy for all supported ops."""
...
class TestEinsum:
    ...
    def test_matmul(self):
        ...
        assert numpy.allclose(
            fnp.einsum("ij,jk->ik", A, B), numpy.einsum("ij,jk->ik", A, B)
        )

    def test_trace(self):
        A = numpy.random.randn(4, 4)
        assert numpy.allclose(fnp.einsum("ii->", A), numpy.einsum("ii->", A))

    def test_batch_matmul(self):
        A = numpy.random.randn(2, 3, 4)
        B = numpy.random.randn(2, 4, 5)
        assert numpy.allclose(
            fnp.einsum("bij,bjk->bik", A, B), numpy.einsum("bij,bjk->bik", A, B)
        )
...
class TestDotMatmul:
    ...
    def test_dot_2d(self):
        ...
        assert numpy.allclose(fnp.dot(A, B), numpy.dot(A, B))
    ...
    def test_matmul_2d(self):
        A = numpy.random.randn(3, 4)
        B = numpy.random.randn(4, 5)
        assert numpy.allclose(fnp.matmul(A, B), numpy.matmul(A, B))
...
class TestFreeOps:
    ...
    def test_constants(self):
        assert fnp.pi == numpy.pi
        assert fnp.inf == numpy.inf
        assert issubclass(fnp.ndarray, numpy.ndarray)


class TestInputCoercion:
    """Verify flopscope accepts scalars, lists, and arrays like NumPy."""

    def test_unary_scalar(self):
        assert numpy.allclose(fnp.sqrt(0.5), numpy.sqrt(0.5))

    def test_unary_python_int(self):
        assert numpy.allclose(fnp.exp(1), numpy.exp(1))

    def test_unary_list(self):
        assert numpy.allclose(fnp.sqrt([0.25, 1.0, 4.0]), numpy.sqrt([0.25, 1.0, 4.0]))

    def test_reduction_scalar(self):
        assert numpy.allclose(fnp.sum(5.0), numpy.sum(5.0))

    def test_reduction_list(self):
        assert numpy.allclose(fnp.sum([1, 2, 3]), numpy.sum([1, 2, 3]))

    def test_clip_list(self):
        assert numpy.allclose(fnp.clip([1, 5, 10], 2, 8), numpy.clip([1, 5, 10], 2, 8))

    def test_dot_lists(self):
        assert numpy.allclose(
            fnp.dot([1, 2, 3], [4, 5, 6]), numpy.dot([1, 2, 3], [4, 5, 6])
        )

    def test_linalg_solve_lists(self):
        A = [[1.0, 2.0], [3.0, 4.0]]
        b = [5.0, 6.0]
        assert numpy.allclose(fnp.linalg.solve(A, b), numpy.linalg.solve(A, b))

    def test_fft_list(self):
        assert numpy.allclose(fnp.fft.fft([1, 2, 3, 4]), numpy.fft.fft([1, 2, 3, 4]))
