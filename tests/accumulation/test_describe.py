...
def _comp(regime_id, shape='mixed'):
    return ComponentCost(
        labels=('i',), va=('i',), wa=(), sizes=(3,),
        m=3, alpha=3, dense_count=3,
        regime_id=regime_id, shape=shape,
...
def test_describe_total_includes_summary_fields():
    ...
    cost = AccumulationCost(
        total=6, mu=3, alpha=3, m_total=3, dense_baseline=3,
        num_terms=2, per_component=(c,),
        fallback_used=False,
    )
    d = cost.describe()
    assert 'total' in d
