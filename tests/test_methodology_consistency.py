"""Verify that analytical formulas in benchmarks match mechestim runtime costs.

For a representative subset of operations (one per benchmark category), this
test runs the mechestim-wrapped operation inside a BudgetContext and verifies
...
def _run_and_get_cost(func, *args, **kwargs) -> int:
    """Run a mechestim function inside a budget context and return the FLOP cost."""
...
class TestSortingConsistency:
    ...
    def test_sort(self):
        n = 1000
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.sort, a)
    ...
    def test_argsort(self):
        n = 1000
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.argsort, a)
...
class TestContractionConsistency:
    """matmul: cost = M*N*K for 2D matrix multiply (FMA_COST=1)."""
    ...
    def test_matmul(self):
        ...
        # which should be 32*32*32 = 32768 (FMA_COST=1)
        expected = m * n * k
        assert runtime_cost == expected, (
...
class TestPointwiseConsistency:
    ...
    def test_sin(self):
        n = 1000
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.sin, a)
    ...
    def test_add(self):
        ...
        a = np.random.rand(n)
        b = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.add, a, b)
        assert runtime_cost == n

    def test_exp(self):
        n = 500
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.exp, a)
        assert runtime_cost == n
...
class TestReductionConsistency:
    ...
    def test_sum(self):
        n = 1000
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.sum, a)
        assert runtime_cost == n

    def test_mean(self):
        n = 1000
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.mean, a)
...
class TestPolynomialConsistency:
    """polyval: cost = m * deg (Horner's method, FMA_COST=1)."""
    ...
    def test_polyval(self):
        ...
        x = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.polyval, p, x)
...
class TestFFTConsistency:
    ...
    def test_fft(self):
        n = 1024
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.fft.fft, a)
...
class TestMiscConsistency:
    ...
    def test_histogram(self):
        ...
        bins = 10
        a = np.random.rand(n)
        runtime_cost = _run_and_get_cost(me.histogram, a, bins=bins)
...
class TestRandomConsistency:
    ...
    def test_standard_normal(self):
        n = 1000
        runtime_cost = _run_and_get_cost(me.random.standard_normal, size=n)
        assert runtime_cost == n

    def test_uniform(self):
        n = 500
        runtime_cost = _run_and_get_cost(me.random.uniform, 0.0, 1.0, size=n)
        assert runtime_cost == n
...
class TestLinalgConsistency:
    ...
    def test_cholesky(self):
        ...
        runtime_cost = _run_and_get_cost(me.linalg.cholesky, a)
