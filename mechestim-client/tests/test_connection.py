"""Tests for mechestim._connection and mechestim._protocol."""
...
class TestProtocol:
    ...
    def test_encode_budget_status(self):
        from mechestim._protocol import encode_budget_status
    ...
    def test_encode_free(self):
        from mechestim._protocol import encode_free
    ...
    def test_decode_response_normalizes_bytes_keys(self):
        ...
        payload = msgpack.packb({"status": "ok", "handle": "abc-123"}, use_bin_type=True)
        result = decode_response(payload)
...
class TestConnectionConfig:
    ...
    def test_default_url(self):
        from mechestim._connection import Connection
        # Clear env var if set
        env = {k: v for k, v in os.environ.items() if k != "MECHESTIM_SERVER_URL"}
    ...
    def test_url_from_env_var(self):
        from mechestim._connection import Connection
        with patch.dict(os.environ, {"MECHESTIM_SERVER_URL": "tcp://localhost:5555"}):
            conn = Connection()
...
class TestConnectionSingleton:
    ...
    def test_get_connection_returns_connection(self):
        from mechestim._connection import get_connection, reset_connection, Connection
        reset_connection()
        conn = get_connection()
        assert isinstance(conn, Connection)

    def test_get_connection_returns_same_instance(self):
        from mechestim._connection import get_connection, reset_connection
        reset_connection()
    ...
    def test_reset_connection_closes_old(self):
        ...
        import mechestim._connection as mod
...
class TestSendRecv:
    ...
    def test_send_recv_raises_on_error_response(self):
        ...
        mock_socket = MagicMock()
        mock_socket.recv.return_value = self._make_response(
            {"status": "error", "error_type": "MechEstimServerError", "message": "boom"}
        )
        conn._socket = mock_socket

        raw = encode_request("op")
        with pytest.raises(MechEstimServerError, match="boom"):
            conn.send_recv(raw)
