...
# Site-packages modules are always allowed (flopscope, zmq, msgpack, etc.)
...
def _is_allowed(name: str) -> bool:
    ...
    # Always allow site-packages (flopscope, zmq, msgpack, etc.)
...
def _disabled(name: str):
    ...
    def _blocked(*args, **kwargs):
        raise RuntimeError(f"{name}() is disabled in the flopscope sandbox")
...
def _restricted_open(file, mode="r", *args, **kwargs):
    ...
    if any(c in str(mode) for c in ("w", "a", "x", "+")):
        raise PermissionError(
            f"write access is disabled in the flopscope sandbox (mode={mode!r})"
        )
    ...
    if not any(path.startswith(prefix) for prefix in _ALLOWED_READ_PREFIXES):
        raise PermissionError(
            f"read access to {file!r} is not allowed in the flopscope sandbox"
        )
