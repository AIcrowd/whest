...
_CLIENT_SRC = (
    Path(__file__).resolve().parent.parent / "mechestim-client" / "src" / "mechestim"
)
...
def _generate_perm_group() -> str:
    ...
    core_path = (
        Path(__file__).resolve().parent.parent / "src" / "mechestim" / "_perm_group.py"
    )
...
def _generate_stats_init() -> str:
    ...
    lines = [_HEADER]
    lines.append(
        '"""mechestim.stats \u2014 statistical distributions submodule proxy."""\n\n'
    )
