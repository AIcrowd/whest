...
def _counted_unary_multi(np_func, op_name: str):
    ...
    def wrapper(x):
        ...
        if isinstance(result, tuple):
            result = tuple(_asflopscope(r) for r in result)
        else:
            result = _asflopscope(result)
        return result
...
def _counted_reduction(
    ...
    def wrapper(a, axis=None, **kwargs):
        ...
        # must be the exact same object (identity). Skip FlopscopeArray wrapping
        # and symmetry tagging when out= is supplied — callers relying on
        # out= are opting out of flopscope's subclass semantics.
