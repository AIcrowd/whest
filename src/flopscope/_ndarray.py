...
shift operators to route through flopscope's FLOP-counted fnp.*
functions.

Because FlopscopeArray inherits from numpy.ndarray, isinstance(x,
numpy.ndarray) returns True. All fnp.* functions return FlopscopeArray.
"""
...
class FlopscopeArray(_np.ndarray):
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, out=None, **kwargs):
        ...
        # Reject multi-output ufuncs explicitly — flopscope wrappers do not
        # currently honor multi-output ``out=(out1, out2)``.
        if ufunc.nout != 1:
            raise NotImplementedError(
                f"ufuncs with nout != 1 are not yet supported on FlopscopeArray "
                f"(operation: {ufunc.__name__}, nout={ufunc.nout}); use the "
                f"equivalent flopscope function (e.g. ``flops.modf(a)``) instead."
            )
        ...
        if method == "__call__":
            ...
            raise NotImplementedError(
                f"ufunc.{method} is not yet supported on FlopscopeArray "
                f"(operation: {ufunc.__name__}); use the equivalent "
                f"flopscope function instead, or open an issue if you need this."
            )
        else:
            whest_fn = None
