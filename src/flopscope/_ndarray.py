...
This module defines FlopscopeArray, a thin numpy.ndarray subclass
that overrides arithmetic, matmul, unary, comparison, bitwise, and
shift operators to route through flopscope.numpy's FLOP-counted ops.

Because FlopscopeArray inherits from numpy.ndarray, isinstance(x,
numpy.ndarray) returns True. All flopscope.numpy ops return
FlopscopeArray.
"""
...
def _me():
    ...
    defined in flopscope/numpy/__init__.py, which itself imports this
    module. Defer the import until first use.
    """
...
class FlopscopeArray(_np.ndarray):
    ...
    related operators route through flopscope's counted me.* functions
    ...
    def __array_wrap__(self, out_arr, context=None, return_scalar=False):
        ...
        True), numpy's view-casting to FlopscopeArray loses the OWNDATA flag.
        We copy the result in that case so the FlopscopeArray correctly reports
        ...
        if (
            isinstance(result, FlopscopeArray)
...
def _asflopscope(x):
    """Convert any array-like to FlopscopeArray, preserving OWNDATA flag.
    ...
    - plain numpy.ndarray that owns its data: copied so OWNDATA is
      preserved on the resulting FlopscopeArray
    - plain numpy.ndarray view (OWNDATA=False): view-cast to FlopscopeArray
    ...
    if isinstance(x, _np.ndarray):
        result = x.view(FlopscopeArray)
        if x.flags.owndata:
            result = result.copy(order="A")
        return result
    arr = _np.asarray(x)
    result = arr.view(FlopscopeArray)
