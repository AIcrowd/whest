...
- **Einsum path caching.** Contraction paths are now cached in a module-level
  LRU cache (default 4096 entries). Repeated <OpRef name="einsum" /> calls with the same
...
- **Enriched `PathInfo` display** — <OpRef name="einsum_path" /> now produces a `PathInfo`
  whose `format_table(verbose=False)`
...
- <OpRef name="dot" /> and <OpRef name="matmul" /> costs are now computed via the einsum cost model instead of separate formulas.
