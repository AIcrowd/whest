...
**Symptom:** Using <OpRef name="fft.fft" /> on real data when <OpRef name="fft.rfft" /> would suffice

**Fix:** <OpRef name="fft.rfft" /> costs half as much. If your input is real-valued, always prefer `rfft`/`irfft` over `fft`/`ifft`.
...
- [Plan Your Budget](./budget-planning) — general cost estimation workflow
- [FLOP Counting Model](../understanding/flop-counting-model) — how all costs are computed
