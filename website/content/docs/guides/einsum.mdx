...
where `μ` is the product of all index dimensions (the FMAs you'd issue) and `α` charges the accumulation across the contracted axes. Each FMA counts as 1 operation; the factor-of-2 you see for dense matmul comes from `α ≈ μ`, not from double-counting FMAs.
...
Path: [(0, 1), (0, 2), (0, 1)]
  ...
  Optimized cost (flopscope):  30,000
                     Speedup:  100.000x
                     Savings:  99.0%
...
The printed table gives you the contraction order, naive-vs-optimized FLOP counts, savings, largest intermediate, grouped index sizes, and one row per pairwise contraction step. Each step shows the chosen contract tuple, subscript, per-step FLOPs, and BLAS tag.
