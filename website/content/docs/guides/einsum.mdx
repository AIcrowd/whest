...
*Use this page to understand `we.einsum` -- the core computation primitive in flopscope.*
...
```python
with we.BudgetContext(flop_budget=10**8) as budget:
    S = we.as_symmetric(we.eye(10), symmetry=we.SymmetryGroup.symmetric(axes=(0, 1)))  # 55 unique elements
    v = we.ones((10,))
...
with we.BudgetContext(flop_budget=10**8) as budget:
    ...
    C = we.einsum('ki,kj->ij', X, X, symmetry=we.SymmetryGroup.symmetric(axes=(0, 1)))
...
n = 10
T = we.as_symmetric(we.ones((n, n, n)), symmetry=we.SymmetryGroup.symmetric(axes=(0, 1, 2)))
