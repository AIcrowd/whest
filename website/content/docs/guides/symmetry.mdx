...
When you want example tensors for arbitrary groups, prefer ``we.symmetrize``.
It is a handy helper that applies the Reynolds operator.
Use ``we.symmetrize`` for randomized example data generation, and
``we.as_symmetric`` when you already have concrete data to validate and tag.

``R_G(T) = (1 / |G|) * sum_{g in G} g · T``

```python
S = we.symmetrize((4, 4, 4), s3_group)
T = we.symmetrize((4, 4, 4), c3_group)
U = we.symmetrize((4, 4, 4, 4), d4_group)
```
...
s3_tensor = we.symmetrize((4, 4, 4), s3_group)
c3_tensor = we.symmetrize((4, 4, 4), c3_group)
d4_tensor = we.symmetrize((4, 4, 4, 4), d4_group)
```

The propagation examples below assume `import whest as we` and
`we.symmetrize(...)` from the earlier sections.
...
s3_tensor = we.symmetrize((4, 4, 4), s3_group)
c3_tensor = we.symmetrize((4, 4, 4), c3_group)
c4_tensor = we.symmetrize((4, 4, 4, 4), c4_group)
