...
```python
s3_group = we.SymmetryGroup.symmetric(3, axes=(0, 1, 2))
...
```python
c3_group = we.SymmetryGroup.cyclic(3, axes=(0, 1, 2))
...
```python
d4_group = we.SymmetryGroup.dihedral(4, axes=(0, 1, 2, 3))
...
```python
row_swap_group = we.SymmetryGroup.symmetric(2, axes=(0, 1))
column_swap_group = we.SymmetryGroup.symmetric(2, axes=(2, 3))
...
s3_group = we.SymmetryGroup.symmetric(3, axes=(0, 1, 2))
c3_group = we.SymmetryGroup.cyclic(3, axes=(0, 1, 2))
c4_group = we.SymmetryGroup.cyclic(4, axes=(0, 1, 2, 3))
...
stretched_s2_tensor = we.as_symmetric(
    we.ones((1, 1, 4)),
    symmetry=we.SymmetryGroup.symmetric(2, axes=(0, 1)),
)
...
- **Unary pointwise ops** preserve symmetry-aware costs, but for
  non-full groups such as `C_k`, `D_k`, or custom groups the current implementation widens the metadata to full symmetry on those axes
