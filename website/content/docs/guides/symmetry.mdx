...
| `we.einsum('ki,kj->ij', x, x, symmetry=we.SymmetryGroup.symmetric(axes=(0, 1)))` | `m * n^2` | `m * n * (n + 1) / 2` | the repeated `x` operand lets flopscope detect symmetric output and reduce the cost |
...
`we.as_symmetric()` validates the data first. After that, Flopscope propagates
...
| `we.add(s3_tensor, c3_tensor)` | `SymmetricTensor` or `FlopscopeArray` | binary pointwise ops keep the intersection of both operands' groups |
...
| `s2_matrix[we.array([0, 1])]` | `FlopscopeArray` | advanced indexing drops symmetry conservatively |
