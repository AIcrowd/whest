...
For multi-operand einsums (3+ operands), use <OpRef name="einsum_path" /> to see the step-by-step contraction breakdown with per-step costs and symmetry savings:
...
<OpRef name="einsum_path" /> does not execute the contraction, but it does record a nominal 1-FLOP planning event so the path query itself is still visible in the operation log.
...
Call `we.budget_summary()` after your computation for a human-readable breakdown, or `budget.summary()` inside a context:
...
The summary shows cost per operation type, sorted by highest cost first. Look for operations consuming a disproportionate share of the budget.
...
Use `we.budget_summary_dict(by_namespace=True)` for per-namespace breakdowns.
...
| `namespace` | Namespace label from the BudgetContext |
...
5. **Split into phases.** Use namespaces to budget different phases independently, so an expensive initialization does not crowd out the main computation:

```python
with we.BudgetContext(flop_budget=init_budget, namespace="init") as b1:
    # initialization
    ...

with we.BudgetContext(flop_budget=solve_budget, namespace="solve") as b2:
    # main computation
    ...

we.budget_summary()  # shows both phases separately
```
