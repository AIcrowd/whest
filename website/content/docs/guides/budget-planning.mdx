...
cost = we.flops.pointwise_cost("exp", shape=(256, 256))
print(f"Pointwise cost: {cost:,}")      # 65,536

# Reduction cost (sum, mean, max, etc.)
cost = we.flops.reduction_cost("sum", input_shape=(256, 256))
...
steps = [
    ...
    ("ReLU (maximum)", we.flops.pointwise_cost("maximum", shape=(256,))),
    ("sum reduction", we.flops.reduction_cost("sum", input_shape=(256,))),
]
