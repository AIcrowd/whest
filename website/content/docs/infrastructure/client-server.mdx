...
<Callout type="info">
This page covers the client-server architecture used for **competition evaluation**, where participant code runs in an isolated container. For how whest wraps NumPy internally, see [How whest Works](/docs/understanding/how-whest-works).
</Callout>

*Use this page to understand how whest's client-server architecture works and why it exists.*

**You will learn:**
- Why whest uses a client-server model for competition evaluation
...
3. **Arrays stay on the server.** The client holds lightweight `RemoteArray` handles that reference server-side data. When you call <OpRef name="einsum" />, the client sends the operation and handle IDs to the server, which executes it and returns a new handle.
...
- [Running with Docker](./docker) — set up client-server locally
- [Contributor Guide](../development/contributing) — source-checkout commands for local development
- [Quickstart](../getting-started/quickstart) — getting started with the local library
