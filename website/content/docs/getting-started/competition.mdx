...
The `namespace` parameter sets the root namespace prefix for that budget context. Nested `flops.namespace(...)` scopes extend it with dotted segments, but they do not create child budgets or split the FLOP limit into separate pools.
