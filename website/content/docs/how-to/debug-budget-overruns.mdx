...
- How to inspect the per-call operation log for detailed diagnostics
- How to use the live budget display for real-time monitoring
- How to reduce FLOP consumption with common strategies
...
Once you've identified the expensive operations, here are common strategies:

- **Reduce dimensions**: If `random.randn(1024, 1024)` is too expensive, try smaller arrays
- **Exploit symmetry**: If operands are symmetric, use `SymmetricTensor` to halve costs (see [Exploit Symmetry](./exploit-symmetry.md))
- **Use cheaper operations**: `we.matmul` is often cheaper than equivalent `we.einsum` for simple cases
- **Increase budget**: If the computation is genuinely needed, raise `flop_budget`
- **Split into phases**: Use namespaces to budget different phases independently
