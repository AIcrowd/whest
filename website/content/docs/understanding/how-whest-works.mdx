...
whest exposes a NumPy-compatible API. When you write `import whest as we` and call `we.einsum(...)`, you get a function that behaves like `np.einsum(...)` but with FLOP counting layered on top.
