---
title: "How whest Works"
---

*Understand how whest wraps NumPy to count every FLOP.*

**You will learn:**
- The wrapping pattern that makes `import whest as we` a drop-in for NumPy
...
whest exposes a NumPy-compatible API. When you write `import whest as we` and call <OpRef name="einsum" />, you get a function that behaves like `np.einsum(...)` but with FLOP counting layered on top.

Under the hood, whest re-exports wrapped versions of NumPy functions. The `whest/__init__.py` module imports from internal modules that each handle a category of operations:
...
- **Submodules** -- `whest.linalg`, `whest.fft`, `whest.random`, `whest.stats`
...
When you call a counted operation, whest computes its FLOP cost analytically from the tensor shapes *before* the operation executes. The cost depends on the operation category:

| Category | Cost formula | Example |
|----------|-------------|---------|
| Pointwise unary | `numel(output)` | <OpRef name="exp" /> on shape (256, 256) costs 65,536 |
| Pointwise binary | `numel(output)` | <OpRef name="add" /> with broadcast output (256, 256) costs 65,536 |
| Reduction | `numel(input)` | <OpRef name="sum" /> on shape (256, 256) costs 65,536 |
| Einsum | product of all index dimensions | `'ij,jk->ik'` with shapes (m, k), (k, n) costs m * k * n |
| Free | 0 | <OpRef name="zeros" />, <OpRef name="reshape" />, <OpRef name="transpose" /> |
...
If no explicit `BudgetContext` is active, whest automatically creates a global default context with a budget of 1e15 FLOPs (configurable via the `WHEST_DEFAULT_BUDGET` environment variable). This means bare calls outside any `with` block still work and still count FLOPs.

### The flow of a single call

Here is what happens when you call <OpRef name="matmul" /> with shapes (100, 200) and (200, 50):

```
User calls we.matmul(A, B)
    |
    v
whest computes cost: 100 * 200 * 50 = 1,000,000 FLOPs
    |
    v
...
The registry (`whest/_registry.py`) is a mapping of every NumPy callable to its classification and cost behavior. Each entry specifies:
...
When per-operation [weights](/docs/understanding/calibration) are loaded, the analytical cost is multiplied by the operation's weight before deduction. This allows the cost model to reflect that `exp` is more expensive than `abs` in terms of actual hardware instructions, while keeping the base formulas simple and deterministic.
