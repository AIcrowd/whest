...
**You will learn:**
- How whest computes FLOP costs analytically from tensor shapes
...
whest loads the packaged official per-operation
...
cost. Set `WHEST_DISABLE_WEIGHTS=1` if you want the pure analytical unit-cost
model instead.
...
Official weights are packaged with whest and enabled by default on import.
Use `WHEST_WEIGHTS_FILE` to override them with a custom JSON file, or set
`WHEST_DISABLE_WEIGHTS=1` to disable weighting entirely and fall back to
...
them at import time, set `WHEST_WEIGHTS_FILE`:
