...
4. **Validate pi:** `pi(V) is a subset of V` and `pi(W) is a subset of W`. Any cycle crossing V to W
   invalidates the sigma.
5. **Collect pi as a `Permutation` object** restricted to V labels (and
   separately to W labels). Non-identity permutations become generators
   of the detected `SymmetryGroup`.

The sigma-loop collects all non-identity pi restrictions as `Permutation` objects.
These generators are passed to Dimino's algorithm (via `SymmetryGroup(...)`)
...
objects (generators) and builds a `SymmetryGroup` directly.
