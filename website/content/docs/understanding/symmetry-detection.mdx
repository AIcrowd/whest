...
> level symmetries are now recovered separately via V-sub × S(W) (below)
...
5. **Collect pi as a `Permutation` object** over all labels. Non-identity
   permutations become generators of the detected per-tuple group `G_PT`.
...
- **Per-tuple** `G_PT`: a permutation π such that for every tuple `t` of
...
- **Expression-level** `G_EXPR`: a permutation π such that the total sum
  is unchanged under renaming all labels by π, even if individual terms get
  reshuffled. Always a superset of `G_PT`; includes dummy-rename
  permutations of summed labels.

The expression-level group factors cleanly (when restricted to V/W-
preserving elements) as **V-sub × S(W)**:

- **V-sub**: `G_PT` projected to V-labels (the action on output axes).
...
`G_EXPR` is computed directly from this formula, with no additional σ-loop
...
**Why both?** Compression formulas (μ, α) must use `G_PT` — feeding them
`G_EXPR` over-compresses, because dummy-rename orbits contain tuples whose
