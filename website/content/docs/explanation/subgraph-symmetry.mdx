---
title: "Subgraph Symmetry Detection"
---

*Use this page to understand the internals of whest's symmetry detection algorithm. This is contributor-level detail -- most users should start with the [Exploit Symmetry](../how-to/exploit-symmetry.md) how-to guide instead.*
...
For a `SymmetricTensor` with subscript `"ij"` and declared symmetry `S₂{i,j}` (S₂ being the symmetric group on 2 elements -- all permutations of `{i,j}`),
...
where σ (a permutation of rows — reordering operand axes) permutes identical-operand rows and π (a permutation of columns — relabeling index names) permutes label columns,
...
detected `PermutationGroup` on labels, closed via Dimino's algorithm (an algorithm that builds a complete group from a set of generators by systematically composing them until no new elements are found).
...
generated group equals S_k (the full symmetric group -- all k! permutations, checked via `order == k!`), the existing
`C(n+k-1, k)` formula applies as before. When the group is a proper subgroup
(e.g., C₃ -- the cyclic group of order 3, containing only rotations -- from `einsum('ij,jk,ki->', A, A, A)`), Burnside's lemma (which counts distinct elements under group symmetry by averaging the number of elements fixed by each group element) gives the
