...
*Use this page if you are an AI coding assistant (or building one) that needs to generate whest code correctly.*

**You will learn:**
- How to orient yourself with llms.txt, ops.json, and the cheat sheet
- The five rules for generating correct whest code
- How to avoid common mistakes AI agents make with whest
...
When in doubt, check `ops.json` or the [API Reference](./).
...
**4. Use <OpRef name="einsum" /> as the primary computation primitive.**
...
- Use `symmetric_axes` for symmetric outputs:
  <OpRef name="einsum" /> with `symmetric_axes=[(0, 1)]`, e.g. `'ki,kj->ij'`
...
|---------|-------------|-----|
| Using `np.einsum` instead of <OpRef name="einsum" /> | FLOPs not counted, budget not checked | Always use `we.*` for operations you want tracked |
...
- [Exploit Symmetry](../guides/symmetry) — detailed symmetry guide
