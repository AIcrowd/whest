...
**3. Use `we.accounting.*` to estimate costs before running.**

```python
cost = we.accounting.einsum_cost('ij,jk->ik', shapes=[(256, 256), (256, 256)])
cost = we.accounting.svd_cost(m=256, n=256, k=10)
```
