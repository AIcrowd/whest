...
```json
{
  "name": "einsum",
  "slug": "einsum",
  "detail_href": "/docs/api/ops/einsum/",
...
- source links (`source.whest`, `source.numpy`)
...
- Use `symmetry=` with explicit groups for symmetric outputs:
  `we.einsum('ki,kj->ij', X, X, symmetry=we.SymmetryGroup.symmetric(axes=(0, 1)))`
- Wrap known-symmetric matrices with `we.as_symmetric(data, symmetry=we.SymmetryGroup.symmetric(axes=(0, 1)))`
