...
A weight of 16.0 for `exp` means each analytical FLOP of `exp` is calibrated as approximately 16 times more expensive than one analytical FLOP of `add` under the chosen measurement mode. In normal use, whest loads packaged official weights automatically on import. Set `WHEST_DISABLE_WEIGHTS=1` if you want the pure analytical unit-cost model instead.
