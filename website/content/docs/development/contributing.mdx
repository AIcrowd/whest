...
| `website/public/ops.json` | Generated API operation manifest consumed by the interactive API reference |
| `scripts/generate_api_docs.py` | Regenerates the API operation manifest |
...
Do not hand-edit `website/public/ops.json`. The interactive API reference page
...
whest functions internally call numpy (e.g., <OpRef name="dot" /> calls `_np.dot`). Since `_np` IS the numpy module, patching `numpy.dot` to route through <OpRef name="dot" /> would cause infinite recursion: <OpRef name="dot" /> → `_np.dot` → `numpy.dot` → <OpRef name="dot" /> → ...
...
| `linalg.outer` | 1 | <OpRef name="linalg.outer" /> delegates to `np.outer` (not `np.linalg.outer`), which has different validation behavior |
