...
3. **Reports symmetry-aware costs.** Each step's cost is reduced by two independent multiplicative factors — an output (V-side) ratio and an inner (W-side) ratio — each computed exactly via Burnside's lemma over the permutation group returned by the oracle. Both the symmetry-reduced cost and the dense cost are reported in `PathInfo`. See [Einsum cost model](../concepts/flop-counting-model.md#einsum-cost-model) for the full derivation.
