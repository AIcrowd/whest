...
export default function ExplorerThemeDock({ explorerThemeId, onChange }) {
  ...
  const warmInkLabel =
    'text-[color:color-mix(in_oklab,var(--explorer-editorial-accent)_45%,var(--explorer-ink))]';

  return (
    <details className="fixed bottom-6 right-6 z-50">
      <summary
        className="list-none cursor-pointer rounded-full border border-[color:color-mix(in_oklab,var(--explorer-editorial-accent)_28%,var(--explorer-border))] bg-white px-4 py-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-foreground shadow-[0_10px_30px_rgba(41,44,45,0.12)] transition-colors hover:border-[var(--coral)] hover:text-[var(--coral)]"
      >
        Explorer Theme
      </summary>
      <div className="mt-3 w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-[color:color-mix(in_oklab,var(--explorer-editorial-accent)_24%,var(--explorer-border))] bg-white p-4 shadow-[0_18px_48px_rgba(41,44,45,0.16)]">
        <div className="mb-3">
          <div className={`text-[10px] font-semibold uppercase tracking-[0.18em] ${warmInkLabel}`}>
            Explorer Theme
          </div>
        ...
        <label className={`block text-[11px] font-semibold uppercase tracking-[0.16em] ${warmInkLabel}`}>
