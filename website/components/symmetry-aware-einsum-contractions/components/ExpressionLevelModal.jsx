          ...
          <section>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <NarrativeCallout label="Formal symmetry group">
                {`$G_{\\text{f}}$ — the subgroup of $\\mathrm{Sym}(V) \\times \\mathrm{Sym}(W)$ consisting of V-preserving label permutations $\\pi = (\\pi_V, \\pi_W)$ under which the output tensor $R$, viewed as a polynomial in the entries of the operand tensors, is invariant: $R[\\pi_V\\,\\omega] = R[\\omega]$ as polynomials, after relabelling the summed indices by $\\pi_W$. "Formal" has its standard mathematical meaning — invariance at the level of the expression treated as a formal polynomial, not at the level of its values on any specific operand.`}
              </NarrativeCallout>
              <NarrativeCallout label="Relationship" tone="algorithm">
                {`$G_{\\text{pt}}$ and $G_{\\text{f}}$ live in different ambient groups. $G_{\\text{pt}} \\subseteq \\mathrm{Sym}(L)$ may include cross-V/W elements — permutations that send a V-label to a W-label — whereas $G_{\\text{f}} \\subseteq \\mathrm{Sym}(V) \\times \\mathrm{Sym}(W)$ is V-preserving by design. They are connected through $G_{\\text{pt}}\\big|_V$, the V-action that $G_{\\text{pt}}$'s V/W-preserving elements induce on V-labels; Section 5 shows that $G_{\\text{pt}}\\big|_V$ is exactly the V-factor of $G_{\\text{f}}$. The strictly-formal elements of $G_{\\text{f}}$ outside that V-factor are dummy-variable renamings of the summed labels ($\\{e\\} \\times S(W)$) that hold only after aggregation. For bilinear-trace (Section 3 below) $G_{\\text{pt}}\\big|_V$ has 2 elements, $S(W)$ has 2 elements, and $G_{\\text{f}}$ has 4.`}
              </NarrativeCallout>
            </div>
          </section>
