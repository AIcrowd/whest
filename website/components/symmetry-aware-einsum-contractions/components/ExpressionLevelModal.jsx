          ...
          <section>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <NarrativeCallout label="Formal symmetry group">
                {`$G_{\\text{f}}$ — the subgroup of $\\mathrm{Sym}(V) \\times \\mathrm{Sym}(W)$ consisting of V-preserving label permutations $\\pi = (\\pi_V, \\pi_W)$ under which the output tensor $R$, viewed as a polynomial in the entries of the operand tensors, is invariant: $R[\\pi_V\\,\\omega] = R[\\omega]$ as polynomials, after relabelling the summed indices by $\\pi_W$. "Formal" has its standard mathematical meaning — invariance at the level of the expression treated as a formal polynomial, not at the level of its values on any specific operand. $G_{\\text{f}}$ decomposes into two factors with distinct detection mechanisms: a row-witnessed V-action (everything that arose from the σ-loop's wreath enumeration) and a row-unwitnessed W-permutation (dummy renaming of summed labels, never surfaced by any row-perm).`}
              </NarrativeCallout>
              <NarrativeCallout label="Relationship" tone="algorithm">
                {`$G_{\\text{pt}}$ and $G_{\\text{f}}$ live in different ambient groups. $G_{\\text{pt}} \\subseteq \\mathrm{Sym}(L)$ may include cross-V/W elements — permutations that send a V-label to a W-label — whereas $G_{\\text{f}} \\subseteq \\mathrm{Sym}(V) \\times \\mathrm{Sym}(W)$ is V-preserving by design. They are connected through $G_{\\text{pt}}\\big|_V$, the V-action that $G_{\\text{pt}}$'s V/W-preserving elements induce on V-labels; Section 5 shows that $G_{\\text{pt}}\\big|_V$ is exactly the V-factor of $G_{\\text{f}}$. The strictly-formal elements of $G_{\\text{f}}$ outside that V-factor are **row-unwitnessed** — dummy-variable renamings of the summed labels ($\\{e\\} \\times S(W)$) that hold only after aggregation and never appear as induced $\\pi$'s of any wreath element. For bilinear-trace (Section 3 below) $G_{\\text{pt}}\\big|_V$ has 2 elements, $S(W)$ has 2 elements, and $G_{\\text{f}}$ has 4.`}
              </NarrativeCallout>
            </div>
          </section>
            ...
            */}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <NarrativeCallout label="The open optimization">
                {`For every $\\sigma \\in G_{\\text{pt}}\\big|_V$, the identity $R[\\sigma\\,\\omega] = R[\\omega]$ holds on the output tensor — cell by cell, not merely in aggregate. For generic operands at $n$ large enough to separate the group's orbits, this inclusion is tight: $G_{\\text{pt}}\\big|_V$ is the complete structural V-symmetry of $R$. At small $n$ the index space itself can degenerate and produce coincidental V-symmetries that vanish at larger $n$ — e.g. $\\texttt{cross-c3-partial}$ at $n = 2$ has $R[0,1] = R[1,0]$ for every cyclic $T$, collapsing only once $n \\geq 3$ separates the cyclic orbits on $\\{i,j,k\\}$. Persistent extra V-symmetry at large $n$ would require value-level structure in the operands (rank-deficiency, sparsity) outside this engine's scope. A computational model with symmetry-aware output storage — where a single physical slot represents all cells in a $G_{\\text{pt}}\\big|_V$-orbit — collapses writes to mirrored cells automatically and reduces the accumulation count.`}
              </NarrativeCallout>
