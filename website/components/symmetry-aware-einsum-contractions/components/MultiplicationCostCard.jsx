 ...
 *   M — orbit count = |X/G|  (computed via size-aware Burnside)
 *   μ — Multiplication Cost = (num_terms − 1) · M
 *   α — Accumulation Cost (a separate quantity; see the Accumulation card)
 *
 * The card shows BOTH quantities: the Burnside formula for M, and how
 * (num_terms − 1) converts it to μ. A hover tooltip on the formula
 * explains the two properties users most need to internalise:
 *   1. M is structure-agnostic — it only sees G's action on L, not the
 *      V/W split, so the same Burnside sum covers every regime (trivial,
 *      all-visible, all-summed, and every mixed regime).
 *   2. μ = (num_terms − 1) · M captures the k-operand einsum cost:
 *      forming one product per orbit rep costs k−1 binary multiplies.
 */
    ...
    >
      ...
      <div className="text-[12px] leading-5 text-gray-300">
        <div className="font-semibold uppercase tracking-wider text-gray-400 text-[10px] mb-1">
          M doesn&apos;t see V vs W
        </div>
        <p>
          The Burnside sum only knows about G acting on the full label set.
          It doesn&apos;t read the V/W split at all — so the exact same formula
          applies to every regime: trivial, all-visible, all-summed, and every
          mixed case. Contraction structure never enters M.
        </p>
      </div>
      ...
      <div className="mt-3 border-t border-gray-700 pt-3 text-[12px] leading-5 text-gray-300">
        ...
        <p>
          M counts orbits — one product <span className="italic">to compute</span> per orbit rep. Each
          product is a chain of <span className="font-mono">num_terms</span> operand entries
          (2-operand einsum → chain of 2, 3-operand → chain of 3, etc.), which costs{' '}
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      <p className="mt-2 text-sm leading-6 text-foreground">
        ...
        number of orbits <span className="font-mono">M</span> once; the
        multiplication cost is then
        <span className="font-mono"> μ = (num_terms − 1) · M</span> — one
        product per orbit, where forming that product costs
        <span className="font-mono"> num_terms − 1</span> binary multiplies.
        No regime dispatch; one formula covers trivial, all-visible, all-summed,
        and every mixed regime.
      </p>

      {/* Hover-wrapped formula — shows μ on top, the M/Burnside identity
          below, so the reader sees how the two connect without leaving
          the card. */}
      <div
        ref={formulaRef}
      ...
      >
        <Latex
          math={String.raw`\mu \;=\; (\texttt{num\_terms} - 1)\,\cdot\, M`}
          display
        />
      ...
      {components.length > 0 && (
        <div className="mt-3 space-y-1.5">
          ...
          {components.map((comp, i) => {
            const orbits = comp.multiplicationCount
              ?? comp.multiplication?.count
              ?? null;
            const mu = orbits != null ? orbits * factor : null;
            return (
              <div
                key={`mult-${i}`}
              ...
              >
                ...
                <span className="ml-auto flex items-center gap-3 font-mono text-gray-900">
                  <span className="text-muted-foreground">
                    M = {orbits != null ? orbits.toLocaleString() : '—'}
                  </span>
                  <span>
                    μ = {mu != null ? mu.toLocaleString() : '—'}
                  </span>
                </span>
              </div>
            );
