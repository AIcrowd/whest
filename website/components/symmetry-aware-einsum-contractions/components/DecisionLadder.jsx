...
const QUESTIONS = [
  ...
  },
  {
    ...
    intuition: 'Examples with $W = \\emptyset$ at the whole-einsum level (every input label appears in the output): $\\texttt{outer}$ ($\\texttt{ab,cd→abcd}$), $\\texttt{triangle}$ ($\\texttt{ij,jk,ki→ijk}$), and $\\texttt{four-cycle}$ ($\\texttt{ij,jk,kl,li→ijkl}$). These are "visible-only" einsums.',
  ...
  },
  {
    ...
    intuition: 'Examples that fire the Young regime: $\\texttt{young-s3}$ ($\\texttt{abc→ab}$ with $T$ fully symmetric, $|G|=6=3!$), $\\texttt{young-s4-v2w2}$, $\\texttt{young-s4-v3w1}$. Example with cross elements but $G \\neq \\mathrm{Sym}$: $\\texttt{cross-c3-partial}$ ($C_3 \\subsetneq S_3$). It falls to brute-force orbit because the closed form requires the full symmetric group on the component\'s label set.',
    onTrue: 'young',
    onFalse: 'bruteForceOrbit',
    stage: 2,
  },
