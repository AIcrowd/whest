...
const QUESTIONS = [
  {
    ...
    intuition: 'Generic operands with no declared axis symmetries, and no repeated-operand structure the σ-loop can exploit, land here. Examples: *frobenius* ($\\texttt{ij,ij→}$ — two identical $A$\'s but matching subscripts kill the swap), *matrix-chain* ($\\texttt{ij,jk→ik}$ — identical $A$\'s but different subscript structure), *mixed-chain* ($A$,$B$,$A$ with $B$ breaking the chain).',
  ...
  },
  {
    ...
    intuition: 'Examples with $W = \\emptyset$ at the whole-einsum level (every input label appears in the output): *outer* ($\\texttt{ab,cd→abcd}$), *triangle* ($\\texttt{ij,jk,ki→ijk}$), and *four-cycle* ($\\texttt{ij,jk,kl,li→ijkl}$). These are "visible-only" einsums.',
  ...
  },
  {
    ...
    intuition: 'Examples with $V = \\emptyset$: *frobenius* ($\\texttt{ij,ij→}$) and *trace-product* ($\\texttt{ij,ji→}$) — both produce scalars. "Yes" here means both $V$ and $W$ are populated and $G$ is non-trivial; this is the only branch that pays the cost of dimino and proceeds into Stage 2.',
  ...
  },
  {
    ...
    intuition: 'Examples: *cross-s3* ($\\texttt{ijk→i}$ with $T$ fully symmetric — $S_3$ on $\\{i,j,k\\}$ reducing to a vector) and *cyclic-cross* (same shape with $C_3$). Any $\\texttt{...→i}$ einsum with non-trivial $G$ that mixes $i$ with summed labels lands here.',
...
function buildLadderLayout(activeLeafIds, spotlightLeafIds) {
  ...
  stage1.forEach((q, i) => {
    ...
    if (i === 0) {
      edges.push({
        ...
        style: { stroke: notationColor('w_summed'), strokeWidth: 1.5 },
      });
    } else {
  ...
  edges.push({
    ...
    style: { stroke: notationColor('g_detected'), strokeWidth: 1.5, strokeDasharray: '6 3' },
  });
...
const DecisionLadderGraph = memo(function DecisionLadderGraph({
  ...
  const clampZoom = useCallback((nextZoom) => Math.max(0.4, Math.min(2.5, nextZoom)), []);
    ...
    <div ref={wrapRef} className="relative">
      ...
      {activeTooltip && (
        ...
        >
          ...
          {activeTooltip.body && (
            <div className="whitespace-normal break-words text-sm leading-6 text-stone-700">
          ...
          {activeTooltip.glossary && (
            <div className="mt-3 whitespace-normal break-words border-t border-stone-200 pt-3 text-[11px] leading-relaxed text-stone-700">
          ...
          {activeTooltip.liveReasons && activeTooltip.liveReasons.length > 0 && (
            <div className="mt-3 whitespace-normal break-words border-t border-stone-200 pt-3 text-[11px] leading-relaxed text-stone-700">
              <div className="mb-1.5 text-[10px] uppercase tracking-wider text-stone-500">
                Live for this example
              </div>
              <ul className="list-none space-y-0.5 font-mono text-[11px] text-stone-700">
