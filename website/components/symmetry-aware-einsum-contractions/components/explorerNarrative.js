...
export const EXPLORER_ACTS = [
  ...
  },
  {
    ...
    introParagraphs: [
      `Let $${notationLatex('l_labels')}$ be the set of index labels in the contraction, with $${notationLatex('l_labels')} = ${notationLatex('v_free')} \\sqcup ${notationLatex('w_summed')}$, where $${notationLatex('v_free')}$ are the labels visible on the output and $${notationLatex('w_summed')}$ are eliminated by summation. We build the bipartite incidence object relating operand-axis classes to labels.`,
      `The incidence matrix $${notationLatex('m_incidence')}$ is a candidate filter, not a proof of symmetry. It records which labels occur in which operand-axis classes, while the semantic data carried alongside it — operand identity, ordered slots, declared input symmetries, and label-size orbits — determines whether a candidate can later be accepted as pointwise valid by this model.`,
    ],
