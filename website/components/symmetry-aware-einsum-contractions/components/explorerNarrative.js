export const EXPLORER_ACTS = [
  ...
  },
  {
    ...
    algorithm: 'The σ-loop emits Sources A (declared axis symmetries) and B (identical-operand swaps) — these produce the per-tuple group G_PT. The expression-level group G_EXPR is then V-sub × S(W): the per-tuple group projected to V-labels, times all permutations of W-labels (dummy-rename symmetries, always valid).',
    produces: 'Two groups — G_PT (per-tuple, drives compression) and G_EXPR (expression-level, counting symmetry) — plus the Sources-A-and-B generator set that Section 4 materializes.',
    ...
    bridge: 'Section 4 takes the G_PT generators, runs Dimino to enumerate the full group, and names it. G_EXPR is computed directly from the V-sub × S(W) formula; no Dimino needed.',
    takeaway: 'Two groups, two jobs. G_PT powers the cost formulas; G_EXPR tells the counting story.',
  },
  {
    id: 'proof',
    navTitle: 'Prove Symmetry',
    heading: 'Materialize and Name the Group',
    question: "Given the σ-loop's generators for G_PT, what's the full enumerated group and what is it called?",
    ...
    algorithm: 'Run Dimino on the Sources A+B generators to materialize every element of G_PT. Classify by order and cycle structure: S_n, C_n, D_n, or custom.',
    produces: 'Fully enumerated G_PT with its standard name. G_EXPR is derived in parallel via V-sub × S(W).',
