...
const section6 = {
  ...
  slots: {
    intro: [
      ...
      p('Accumulation is governed by $G_{\\mathrm{pt}}$. Output storage is governed by $G_{\\mathrm{out}}$. Once an output value has been computed, all output cells in the same $G_{\\mathrm{out}}$-orbit contain the same value. A storage layout may therefore keep one representative per output orbit and recover the mirrored entries by symmetry.'),
    ...
    footer: [
      p('The $\\alpha$ shown on the main page counts distinct accumulation operations in the enumerate-and-accumulate evaluation model with $G_{\\mathrm{pt}}$ as the summand-value equivalence relation.'),
