...
const section6 = {
  ...
  slots: {
    intro: [
      ...
      p('$P_{\\mathrm{typed}}(L)$ is the set of equality patterns that only merge same-domain labels.'),
      p('$b_s(\\tilde{x})$ is the number of blocks of domain class $s$.'),
      p('$(n_s)_{b_s}$ is the falling factorial, counting injective labelings for those blocks.'),
      p('$\\overline{G}_{\\tilde{x}}$ is the induced action of the stabilizer of $\\tilde{x}$ on its blocks — the image on blocks, not the raw stabilizer order.'),
      p('$A_{\\tilde{x}}$ is the set of maps from output positions to input equality blocks obtained by moving through the product orbit.'),
      p('$|A_{\\tilde{x}}/H|$ counts the stored output representatives reached by any product orbit above that pattern.'),
    ],
    footer: [
