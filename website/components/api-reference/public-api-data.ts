...
function resolveConcretePublicApiEntry(
  ...
  const operation = operations.find(
    (entry) => entry.flopscope_ref === name || entry.name === name,
  );
  if (!operation?.href) return null;

  return {
    kind: 'operation',
    title: operation.flopscope_ref,
    href: operation.href,
    description: operation.notes,
    namespace_import_path: namespaceForSymbol(operation.flopscope_ref),
  };
}
...
function getOperationEntriesForArea(
  ...
  return getPublicApiOperations()
    ...
    .sort((left, right) => left.flopscope_ref.localeCompare(right.flopscope_ref))
    .map((operation) => ({
      href: operation.href!,
      import_path: operation.flopscope_ref,
